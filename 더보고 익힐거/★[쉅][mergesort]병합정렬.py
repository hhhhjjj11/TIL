...
# 실행순서 
# 1번 phase1 -> 2번 phase1 -> ... -> last-1번 phase1 -> 
# Last번째f -> 걍리턴.(배열길이1) ->
# last-1번째f phase2 -> last-2번째 phase2 -> last-3번째 phase2 -> ... -> 첫번째f phase2

# 재귀함수 패턴 이해하기
# 1. 일단 재귀호출한것을 변수에 담는 경우와 담지 않는경우로 나누어서 생각하기.
#   - 재귀쓴것을 변수에 담는다 -> 리턴이 무조건 존재해야만 함. 그리고 리턴받아서 뭔짓함.
#   - 변수에 안담고 실행만 한다 -> 어떤 하나에 대한 어떤 동작(병합,분할 등)을 실행.
# 2. 리턴문 다음 줄에 재귀호출 
#   - 리턴문의 조건에 걸릴때까지 재귀가 호출됨. -> 조건에 걸려서 리턴되는 함수가 마지막으로 호출되는 함수.
#   - 마지막으로 호출되는 함수는 걍 시작하자마자 끝나버림.
#   - 따라서 마지막-1 번째 함수에서 처음으로 호출 이후의 로직이 샐행됨.
#
# def f(x1):
#   if p:
#       return 
...
#   이후 로직.
#   
# 함수실행(1번) -> 재귀호출(2번) -> 재귀호출(3번) -> 재귀호출(4번) -> ... -> 조건에 걸려서 리턴(Last번) -> last-1번에서 리턴받아서 이후로직실행
# -> 이후로직실행(last-2번) -> 이후로직실행(last-3번) -> ... -> 이후로직실행(1번)
...
def merge_sort(low, high):
    if high - low < 2:  #왜 2인지 모르겠다. 1이어야 할 것 같은데. (high에 len(arr)넣을거라그런듯)
        return
    mid = (low + high) // 2
    merge_sort(low, mid)
    merge_sort(mid, high)
